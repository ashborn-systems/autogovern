...
autogovern/
  src/autogovern/
    ...
    verify/             # reserved for future quality-feedback features
...
- `frameworks/agentic-business-case/` - supplementary content, copied unmodified and scoped by `pack.yaml`: `rubric.md` (quality criteria — risks mapped one-to-one to actual capabilities, kill switch and logging present, strategic restraint named, numbers carrying stated assumptions — available as a reference for future quality-feedback features) and `frameworks.md` (sections 3 and 4 - the 7-point agentic viability checklist and the rule of ten - feed the system card's intended-purpose justification; sections 9-16 duplicate the governance knowledge base and are out of scope, with the governance file authoritative on any conflict)
...
| Attention ledger | `ATTENTION.md` | Informational: notes whether docs are specific (context loaded) or generic (vanilla mode). Points to `init` when generic |
...
The docs are generated from the scan and match reality by construction. There is no second LLM pass that audits the first one's output — a self-audit that routes its fallout to humans is the wrong design. The generation prompt is the only place quality gets enforced: it embeds the style authority's banned-constructions rules and receives only the section's declared inputs.

### Attention ledger

`ATTENTION.md` is informational, not a work queue. It notes whether the run had a context manifest (specific docs) or ran in vanilla mode (generic docs). In vanilla mode it points the user at `autogovern init`. There are no open/close items, no verifier-driven gaps, no claim-stripping. The ledger being empty means a context manifest was loaded; it does not mean the docs were audited.

## Vanilla mode

`autogovern generate` works without `init`. Provider settings come from `AUTOGOVERN_API_BASE`, `AUTOGOVERN_MODEL`, and `AUTOGOVERN_API_KEY_ENV` environment variables; context uses conservative defaults. Docs are generated from the scan alone and are generic. Running `init` writes a context manifest and the same docs become specific to the organisation. Progressive enhancement, not a gate: the more context you provide, the better the docs; the tool never refuses to run because context is missing.
...
Run manifests are the audit trail. Humans never read hashes; `autogovern explain <doc>` renders provenance for any document in plain language.
...
autogovern explain <doc> # plain-language provenance for any document
...
- **Deploy gate (optional)** - a final `autogovern check --strict` before the deploy job, so nothing ships with stale documentation. This gate is the seam AgentGuard extends in enterprise: same position in the pipeline, evaluation upgraded from doc currency to full policy-as-code criteria with suggested fixes
...
6. `generate` runs without `init` (vanilla mode): provider from env vars, generic context defaults, docs produced. With `init`, the same docs are specific. The tool never refuses to run because context is missing
