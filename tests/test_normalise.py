...
def _context(
    *, deployment="internal", autonomy="human-in-the-loop", risk="conservative"
) -> ContextManifest:
    return ContextManifest(
        project=ProjectContext(organisation="x", sector="x", risk_appetite=risk),
        agent=AgentContext(deployment_context=deployment, autonomy_level=autonomy),
    )
...
def test_direct_resolution_skips_llm() -> None:
    ...
    result = normalise_context(context, provider)
...
def test_llm_resolves_multi_value_deployment_context() -> None:
    ...
    context = _context(deployment="customer-facing, internal")
    result = normalise_context(context, provider)
    assert result.deployment_context == DeploymentContext.CUSTOMER_FACING
...
def test_llm_resolves_multi_value_autonomy() -> None:
    ...
    context = _context(autonomy="fully-autonomous, human-on-the-loop")
    result = normalise_context(context, provider)
    assert result.autonomy_level == AutonomyLevel.FULLY_AUTONOMOUS
...
def test_llm_resolves_free_text_risk_appetite() -> None:
    ...
    context = _context(risk="conservative but leaning balanced")
    result = normalise_context(context, provider)
...
def test_llm_fallback_on_provider_error() -> None:
    ...
    context = _context(deployment="something weird", autonomy="also weird")
    result = normalise_context(context, provider)
    assert result.deployment_context == DeploymentContext.CUSTOMER_FACING
    assert result.autonomy_level == AutonomyLevel.FULLY_AUTONOMOUS
    assert result.risk_appetite == RiskAppetite.AGGRESSIVE
...
def test_whitespace_tolerant_direct_resolution() -> None:
    ...
    provider = MagicMock()
    context = _context(deployment="  internal  ", autonomy=" fully-autonomous ")
    result = normalise_context(context, provider)
